...
class TestApp(EWrapper, EClient):
    ...
    def historicalData(self,reqId,bar):
        print("HistoricalData. ",reqId,
                ...
                "Volume:",bar.volume,
                "Count", bar.barCount,
                "WAP:",bar.average) #En esta concatenacion de argumentos, el orden es segun su descripcion en la documentacion
        self.disconnect()#Con esto corto la comunicacion
...
#idRequest,contract,EndDay(Se puede especificar el dia que queremos data)",Duration,Bar(size),typeData(BID,ASK,etc),0(trading hours),1(format data), Bool,Attribute
    ...
    app.run()
    #app.disconnect() #No llega a ejecutar esta linea por alguna razon
